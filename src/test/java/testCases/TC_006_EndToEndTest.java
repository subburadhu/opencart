...
public class TC_006_EndToEndTest extends BaseClass {
	...
	//hp.clickMyAccount();
	hp.clickOnMyAccount(); //TC006 on Home Page
	hp.clickHomePageLogin();
	
	LoginPage lp=new LoginPage(driver);
	lp.setInputTxtEmail(email);
	lp.setInputTxtPassword("test123");
	...
	hp.enterProductName(properties.getProperty("searchProductName"));
	hp.clickSearch();
	
	SearchPage sp=new SearchPage(driver);
	
	if(sp.isProductExist(properties.getProperty("searchProductName")))
	{
		sp.selectProduct(properties.getProperty("searchProductName"));
		sp.setQuantity("1");
		sp.addToCart();
	...
	myassert.assertEquals(totprice, "$123.20");   //validation
